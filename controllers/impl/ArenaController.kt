...
Classe que contém toda a dinamica do jogo,
invocação de métodos e composição de modelos
além disso, aqui é definido as regras e inicio
e fim das rodadas.
*/
...
class ArenaController : AbstractArena() {
    ...
    private fun playerOnesTurn(player: PlayerController, enemy: PlayerController) {
        ...
        when (option) {
            1 -> {
                ...
                try {
                    val success: Boolean = player.atack(a, b, c)
                    if (success) {
                        println("Player2 -> Pontuacao = ${this.playerTwo.getPunctuation()}")
                        this.playerTwo.setPunctuation(
                                player.getCurrentPlayer().getCardsInGame(b).getAtack()
                        )
                        println(
                                "Player2 -> Pontuacao apos ataque = ${this.playerTwo.getPunctuation()}"
                        )
                        ...
                        println(
                                "Player1 -> Pontuacao apos ataque = ${this.playerOne.getPunctuation()}"
                        )
                        this.playerTwo.setCardsDestroyed(
                                player.getCurrentPlayer().getCardsInGame(b)
                        )
                        this.finish()
                        this.startNewRound()
                    }
            ...
            2 -> {
                ...
                val cardEnemy = this.playerTwo.getCardsInGame(c)
                try {
                    val success: Boolean = player.defend(a, b, c)
                    if (success) {
                        ...
                        println(
                                "1)Player2 -> Pontuacao apos defesa = ${this.playerTwo.getPunctuation()}"
                        )
                        this.playerTwo.setCardsDestroyed(cardEnemy)
                        this.startNewRound()
                    } else {
                        ...
                        println(
                                "1)Player1 -> Pontuacao apos defesa = ${this.playerOne.getPunctuation()}"
                        )
            ...
            1 -> {
                ...
                try {
                    ...
                    if (success) {
                        ...
                        println(
                                "2)Player1 -> Pontuacao apos ataque = ${this.playerOne.getPunctuation()}"
                        )
                        ...
                        println(
                                "2)Player2 -> Pontuacao apos ataque = ${this.playerTwo.getPunctuation()}"
                        )
                        this.playerTwo.setCardsDestroyed(
                                player.getCurrentPlayer().getCardsInGame(b)
                        )
            ...
            2 -> {
                ...
                try {
                    ...
                    if (success) {
                        ...
                        println(
                                "Player1 -> Pontuacao apos defesa = ${this.playerOne.getPunctuation()}"
                        )
                        this.playerOne.setCardsDestroyed(cardEnemy)
                        this.startNewRound()
                    } else {
                        ...
                        println(
                                "Player2 -> Pontuacao apos defesa = ${this.playerTwo.getPunctuation()}"
                        )
                ...
                try {
                    if (!this.newRoundP1) {
                        ...
                        if (o in options) {
                            ...
                            val oo = readln().toInt()
                            println(
                                    "P1: Informacoes da carta antes: ${player.getCardsInGame(oo).getName()} A:${player.getCardsInGame(oo).getAtack()} D:${player.getCardsInGame(oo).getDefense()}"
                            )
                            if (c.getAtack() != 0) {
                            ...
                            println(
                                    "P1: Informacoes da carta depois: ${player.getCardsInGame(oo).getName()} A:${player.getCardsInGame(oo).getAtack()} D:${player.getCardsInGame(oo).getDefense()}"
                            )
                            println("Player1 Equipado")
                            this.newRoundP1 = true
                            player.setCardsDestroyed(
                                    c
                            ) // a carta de equipamento é destruida, ao ser equipada
                            this.showPlayersCards(
                                    this.playerOne.getCardsInGame(),
                                    this.playerTwo.getCardsInGame()
                            )
                ...
                try {
                    if (!this.newRoundP2) {
                        ...
                        if (o in options) {
                            ...
                            val oo = readln().toInt()
                            println(
                                    "P2: Informacoes da carta antes: ${player.getCardsInGame(oo).getName()} A:${player.getCardsInGame(oo).getAtack()} D:${player.getCardsInGame(oo).getDefense()}"
                            )
                            if (c.getAtack() != 0) {
                                player.getCardsInGame(oo).setAttack(c.getAtack())
                                player.setCardsDestroyed(
                                        c
                                ) // a carta de equipamento é destruída, ao ser equipada
                            } else {
                                player.getCardsInGame(oo).setDefense(c.getDefense())
                                player.setCardsDestroyed(
                                        c
                            ...
                            println(
                                    "P2: Informacoes da carta depois: ${player.getCardsInGame(oo).getName()} A:${player.getCardsInGame(oo).getAtack()} D:${player.getCardsInGame(oo).getDefense()}"
                            )
    ...
    // exibe a lista de opções para o jogador 1
    ...
    // exibe a lista de opções para o jogador 2
    ...
    override fun choiceYourPlayer(): Array<Int> {
        ...
        var optionTwo = 0
        if (optionOne == 1) optionTwo = 2 else optionTwo = 1
        arr[0] = optionOne
        arr[1] = optionTwo
    ...
    private fun showPlayersCards(cardP1: MutableList<Card>, cardP2: MutableList<Card>) {
        ...
        for ((index, card) in cardP1.withIndex()) {
            if (card.getCategory() == "monstro") {
                ...
                if (card.getModeAtack()) println("EM MODO DE ATAQUE")
                else println("EM MODO DE DEFESA")
