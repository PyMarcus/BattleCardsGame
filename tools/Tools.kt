...
class Tools {
    companion object {
        ...
        private fun readCSVCards(): List<String> {
            ...
            val streamDados: InputStream =
                    File(
                                    basePath +
                                            "/app/src/main/java/br/com/ifmg/battlecardgame/resources/cartas.csv"
                            )
                            .inputStream()
            val leitorStream = streamDados.bufferedReader()
            return leitorStream.lineSequence().filter { it.isNotBlank() }.toList()
        }
